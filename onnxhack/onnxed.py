...
onnx_ops = importlib.import_module('onnxhack.onnx_ops')
...
def get_run_onnx(onnx_model: ModelProto):
  ...
  def buffer_parse(inp: TensorProto) -> Tensor:
    ...
    if len(inp.raw_data) > 0:
      return Tensor(np.frombuffer(inp.raw_data, dtype=tensor_dtype_to_np_dtype(inp.data_type)).astype(dtype_to_np(dtype)).copy(), requires_grad=False).reshape(tuple(inp.dims))
